...
// Copyright (c) 2016-2017 Centurion
...
const string strMessageMagic = "centurion Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    while (bnLowerBound + CENT <= bnUpperBound)
    {
        ...
        if (fDebug && GetBoolArg("-printcreation", false))
            printf("GetProofOfWorkReward() : lower=%ld upper=%ld mid=%ld\n", bnLowerBound.getuint64(), bnUpperBound.getuint64(), bnMidValue.getuint64());
    ...
    if (fDebug && GetBoolArg("-printcreation", false))
        printf("GetProofOfWorkReward() : create=%s nBits=0x%08x nSubsidy=%ld\n", FormatMoney(nSubsidy).c_str(), nBits, nSubsidy);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (fJustCheck)
        ...
        nTxPos = pindex->nBlockPos
            + pindex->GetBlockHeader().GetSerializeSize(SER_DISK, CLIENT_VERSION)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "centurion", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Centurion - December 2016";
        CTransaction txNew;
        txNew.nTime = 1480833198;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x82833e0f44ce03d585230adf7f5dc53ec5bfd634fbbd99be11348129b22ba0dd"));
        block.print();
