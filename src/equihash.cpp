    ...
    while (X.size() > 1) {
        ...
        for (int i = 0; i < X.size(); i += 2) {
            if (!HasCollision(X[i], X[i+1], CollisionByteLength)) {
                return error("IsValidSolution: invalid collision length between StepRows");
            }
            if (X[i+1].IndicesBefore(X[i], hashLen, lenIndices)) {
                return error("IsValidSolution: Index tree incorrectly ordered");
            }
            if (!DistinctIndices(X[i], X[i+1], hashLen, lenIndices)) {
                return error("IsValidSolution: duplicate indices");
            }
